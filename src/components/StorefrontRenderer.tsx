...
import { Product, StoreConfig, StoreOrder, OrderItem } from '../types';
import { 
  ShoppingCart, Heart, Plus, Trash2, ShieldCheck, Cpu, Terminal, 
  ArrowRight, CheckCircle, Globe, Wallet, MessageSquare, Mail, 
  CreditCard, Loader2, RefreshCw, Layers,
  Search, User, Package, X, ShoppingBag, Minus, ArrowLeft, MessageCircle, Check
} from 'lucide-react';
...
// Global conversion multipliers relative to USD base
const CURRENCY_CONVERSION: Record<string, { rate: number; symbol: string }> = {
  USD: { rate: 1.0, symbol: '$' },
  EUR: { rate: 0.92, symbol: '€' },
  PKR: { rate: 278.0, symbol: '₨' },
  GBP: { rate: 0.79, symbol: '£' },
  JPY: { rate: 158.0, symbol: '¥' }
};

// Global country specific tax & shipping charges
const COUNTRY_RULES: Record<string, { taxRate: number; flatShipping: number; currency: 'USD' | 'EUR' | 'PKR' | 'GBP' | 'JPY' }> = {
  'United States': { taxRate: 0.085, flatShipping: 15, currency: 'USD' },
  'United Kingdom': { taxRate: 0.20, flatShipping: 10, currency: 'GBP' },
  'Pakistan': { taxRate: 0.18, flatShipping: 350, currency: 'PKR' },
  'Germany': { taxRate: 0.20, flatShipping: 12, currency: 'EUR' },
  'Japan': { taxRate: 0.10, flatShipping: 1200, currency: 'JPY' },
  'United Arab Emirates': { taxRate: 0.05, flatShipping: 25, currency: 'USD' }
};
...
export default function StorefrontRenderer({ config, isPreview = false, onOrderPlaced }: StorefrontRendererProps) {
  const { id, name, tagline, theme, products, layoutBlocks, currency: storeBaseCurrency, gatewayType, gatewayValue } = config;
  
  const [cart, setCart] = useState<CartItem[]>([]);
  const [isCartOpen, setIsCartOpen] = useState(false);
  const [checkoutStep, setCheckoutStep] = useState<'idle' | 'form' | 'p2p_processing' | 'success'>('idle');
  ...
  const [offlineEngineSynced, setOfflineEngineSynced] = useState(false);
  const [isProcessingP2P, setIsProcessingP2P] = useState(false);

  // Active client-side currency select
  const [clientCurrency, setClientCurrency] = useState<'USD' | 'EUR' | 'PKR' | 'GBP' | 'JPY'>('USD');

  // Selected variations builder state mapped: productID -> { Name: Option }
  const [chosenProductOptions, setChosenProductOptions] = useState<Record<string, Record<string, string>>>({});

  // 1. [Feature 1] Offline first cache engine recovery
  useEffect(() => {
    const cacheKey = `fscom_offline_cart_${id}`;
    const cachedCart = localStorage.getItem(cacheKey);
    if (cachedCart) {
      try {
        setCart(JSON.parse(cachedCart));
      } catch (err) {
        console.warn('Initializing empty cart sequence.');
      }
    }
    setOfflineEngineSynced(true);
  }, [id]);

  // Handle cart cache synchronization
  useEffect(() => {
    if (offlineEngineSynced) {
      localStorage.setItem(`fscom_offline_cart_${id}`, JSON.stringify(cart));
    }
  }, [cart, id, offlineEngineSynced]);

  // Sync client default currency with active database base currency setting on load
  useEffect(() => {
    if (storeBaseCurrency) {
      setClientCurrency(storeBaseCurrency);
    }
  }, [storeBaseCurrency]);

  // Inbound custom styles override applicator
  useEffect(() => {
    const styleId = `store-custom-styles-${id}`;
  ...
  // Safe client side javascript execute scope
  useEffect(() => {
    if (theme.customJS) {
      try {
        const runner = new Function('storeId', 'storeName', theme.customJS);
        runner(id, name);
      } catch (err) {
        console.warn('Store custom script exception ignored.');
      }
    }
  }, [theme.customJS, id, name]);

  // Font typography class selection helper
  const getFontClass = () => {
  ...
  // Convert and format custom price tags dynamically based on chosen currencies
  const formatCostValue = (usdPrice: number) => {
    const clientRule = CURRENCY_CONVERSION[clientCurrency];
    const rateFactor = clientRule.rate;
    const computed = usdPrice * rateFactor;
    return `${clientRule.symbol}${Math.round(computed).toLocaleString()}`;
  };

  const formatCostValueRaw = (usdPrice: number) => {
    const clientRule = CURRENCY_CONVERSION[clientCurrency];
    return Math.round(usdPrice * clientRule.rate);
  };

  // Cart operations
  const handleProductOptionSelection = (productId: string, variantName: string, option: string) => {
    setChosenProductOptions(prev => ({
      ...prev,
      [productId]: {
        ...(prev[productId] || {}),
        [variantName]: option
  ...
  // Get selected options or fallback to default first option for each variant
  const getSatisfiedOptionsForProduct = (product: Product): Record<string, string> => {
    const specified = chosenProductOptions[product.id] || {};
    const filled = { ...specified };
    
    if (product.variants) {
      product.variants.forEach(v => {
        if (!filled[v.name] && v.options && v.options.length > 0) {
          filled[v.name] = v.options[0];
        }
      });
    }
    return filled;
  ...
  const handleAddToCart = (product: Product) => {
    const selectedVariants = getSatisfiedOptionsForProduct(product);
    
    setCart((prev) => {
      // Find item with same productId and identical variant options
      const isMatching = (item: CartItem) => {
        if (item.product.id !== product.id) return false;
        return JSON.stringify(item.selectedVariants) === JSON.stringify(selectedVariants);
      };

      const existingIndex = prev.findIndex(isMatching);
      if (existingIndex > -1) {
  ...
  const handleQuantityUpdate = (productId: string, variantsStr: string, delta: number) => {
    setCart((prev) =>
      prev
        .map((item) => {
          const checkStr = JSON.stringify(item.selectedVariants);
          if (item.product.id === productId && checkStr === variantsStr) {
  ...
  const handleRemoveItem = (productId: string, variantsStr: string) => {
    setCart((prev) =>
      prev.filter((item) => !(item.product.id === productId && JSON.stringify(item.selectedVariants) === variantsStr))
    );
  };
  ...
  const totalWeightCostBase = cart.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
  
  // Tax & flat shipping bounds based on selected shipping country
  const activeCountryRule = COUNTRY_RULES[selectedCountry] || COUNTRY_RULES['United States'];
  const taxSurchargePercent = activeCountryRule.taxRate;
  
  // Convert standard flat shipping back into base units for proper sum accumulation
  const exchangeRate = CURRENCY_CONVERSION[clientCurrency].rate;
  const flatShippingInUSD = activeCountryRule.flatShipping / exchangeRate;
  
  const computedTaxUSD = totalWeightCostBase * taxSurchargePercent;
  const totalCombinedInvoiceUSD = totalWeightCostBase + computedTaxUSD + flatShippingInUSD;

  // Checkout submission handler compiling P2P direct redirection packages
  const executeP2PCheckoutSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsProcessingP2P(true);
    setCheckoutStep('p2p_processing');

    const orderId = 'ORD-' + Math.floor(1000 + Math.random() * 9000);
    const convertedInvoiceAmount = formatCostValue(totalCombinedInvoiceUSD);

    // Assemble unified transaction elements
    const newOrderPayload: StoreOrder = {
      id: orderId,
      customerName: customerName,
      customerEmail: customerEmail,
      customerAddress: `${customerAddress}, ${selectedCountry}`,
      items: cart.map(item => ({
      ...
      totalAmount: Math.round(totalCombinedInvoiceUSD),
      status: 'Pending',
      timestamp: new Date().toISOString()
    };

    // Compile pretty message body
    const itemsLines = cart.map((item) => {
      const variantsDesc = Object.entries(item.selectedVariants)
        .map(([k, v]) => `${k}:${v}`)
        .join(', ');
      return `- ${item.product.name} x ${item.quantity} ${variantsDesc ? `(${variantsDesc})` : ''} [${formatCostValue(item.product.price * item.quantity)}]`;
    }).join('\n');

    const rawMessageBody = `⚡ *FSCOMMERCE P2P SECURE ORDER GATEWAY* ⚡\n\n` +
      `*Store Context:* ${name}\n` +
      `*Order Identifier:* ${orderId}\n` +
      `*Operator:* ${customerName}\n` +
      `*Delivery Mailbox:* ${customerEmail}\n` +
      `*Secure Location:* ${customerAddress}, ${selectedCountry}\n\n` +
      `=============================\n` +
      `*ASSEMBLED CORE NODES:*\n${itemsLines}\n` +
      `=============================\n` +
      `*Tax surcharge:* ${formatCostValue(computedTaxUSD)} (${(taxSurchargePercent * 100).toFixed(1)}%)\n` +
      `*Flat shipping:* ${formatCostValue(flatShippingInUSD)}\n` +
      `*TOTAL DIRECT BILL:* ${convertedInvoiceAmount}\n\n` +
      `⚠️ _Transaction executed over localized hardware layers. Confirming device availability immediately._`;

    // Process after standard mock compilation delay
    setTimeout(() => {
      if (onOrderPlaced) {
        onOrderPlaced(newOrderPayload);
      }

      setIsProcessingP2P(false);
      ...
      if (!isPreview) {
        if (gatewayType === 'whatsapp') {
          const waUrl = `https://api.whatsapp.com/send?phone=${encodeURIComponent(gatewayValue)}&text=${encodeURIComponent(rawMessageBody)}`;
          window.open(waUrl, '_blank');
        } else if (gatewayType === 'email_sys') {
          const mailUrl = `mailto:${encodeURIComponent(gatewayValue)}?subject=${encodeURIComponent(`FSCommerce Order Node [${orderId}]`)}&body=${encodeURIComponent(rawMessageBody)}`;
  ...
  const accentText = { color: theme.accentColor };
  const accentBorder = { borderColor: theme.accentColor };
  const accentBackground = { backgroundColor: theme.accentColor };
            ...
            <div className="w-full h-[300px] md:h-[450px] lg:h-[550px] rounded-2xl overflow-hidden relative group">
              ...
              <div className="absolute inset-0 z-20 flex flex-col justify-center items-center text-center p-6 md:p-12">
                ...
                <h2 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 drop-shadow-xl" style={{ fontFamily: theme.typography === 'playfair' ? 'Playfair Display' : 'Inter' }}>
                  {tagline || 'Discover Premium Quality'}
                </h2>
